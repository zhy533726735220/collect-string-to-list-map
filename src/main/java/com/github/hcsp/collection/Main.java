...
public class Main {
    ...
    public static Map<String, List<User>> collect(List<User> users) {
        Map<String, List<User>> mapList = new TreeMap<String, List<User>>();
        for (User user : users) {
            String department = user.getDepartment();
            List<User> currentList = mapList.get(department);
            if (currentList == null) {
                currentList = new ArrayList<User>();
            }
            currentList.add(user);
            currentList.sort(new Comparator<User>() {
                @Override
                public int compare(User o1, User o2) {
                    return o1.getAge() - o2.getAge();
                }
            });
            mapList.put(user.getDepartment(), currentList);
        }
        return mapList;
    }
